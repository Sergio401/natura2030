#!/usr/bin/env node
// Change executor worker. Long-running process (pm2 app "natura-agent"),
// cwd = web/, user sergio. Polls for the single open job (see
// agent/lib/store.mjs) and drives it through the state machine documented in
// docs/AGENT_CONTRACT.md. Claude never runs shell commands; verification,
// git and pm2 are all driven here, deterministically.
//
// Exports `processOnce(config)` so agent/test/smoke.test.mjs can run the
// state machine in-process without going through the poll loop or the lock.
...
import { ALLOWED_FILES, isAllowed } from './lib/allowlist.mjs';
import {
  resetWorktreeToProd,
  statusPaths,
  diffStat,
...
import { run, tailLines, ShellError } from './lib/shell.mjs';
 ...
 * @returns {Promise<import('./lib/store.mjs').Job|null>} the job after this pass, or null if none was open.
 */
